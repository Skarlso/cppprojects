...
std::map<std::string, verylong> registers = {{"a", 7}, {"b", 0}, {"c", 0}, {"d", 0}};
...
int main(int argc, char* argv[]) {
    ...
    for (auto it = instructions.begin(); it != instructions.end();) {
        ...
        if (it->at(0) == "cpy") {
            ...
            verylong reg = int_or_reg(it->at(1));
            verylong value = int_or_reg(it->at(2));
            ...
            verylong value = int_or_reg(it->at(1));
            auto nxt = it;
            if (value > 0) nxt = std::next(it, value);
            if (value < 0) nxt = std::prev(it, std::abs(value));
            if (nxt >= instructions.end()) {
